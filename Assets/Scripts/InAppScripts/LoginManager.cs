...
public class LoginManager : MonoBehaviour
{
            ...
            signinResponse = JsonConvert.DeserializeObject<SignInResponse>(response);
              if (signinResponse.isSuccess)
              {
                  if (signinResponse.result.IsSubscribed && ReferenceManager.instance.iAPManager.m_StoreController != null && ReferenceManager.instance.iAPManager.m_StoreController.products.all.Length != 0)
                  {
                      ...
                      {
                          ReferenceManager.instance.PopupManager.Show("Signin Failed!", $"Your Subscription has Ended and you need to subscribe"/*, new IOSNativeAlert.AlertButton("ok", () => { ReferenceManager.instance.IAPPAnel.SetActive(true); })*/);
                          return;
                      }
                  }
              ...
              if (signinResponse.isError)
              {
                  ...
                  {
                      ReferenceManager.instance.PopupManager.Show("Signin Failed!", $"Your Trial has Ended and you need to subscribe"/*, new IOSNativeAlert.AlertButton("ok", () => { ReferenceManager.instance.IAPPAnel.SetActive(true); })*/);
                  }
                  else
                  {
    ...
    public void ResetPassword()
    {
        ...
        onSuccess: async (response) =>
        {
            ...
            if (codeValidateReponse.isSuccess)
            {
                ReferenceManager.instance.PopupManager.Show("Password is Reset!", "Your Password Is Reset Successfully"/*, new IOSNativeAlert.AlertButton("Thank You", () => { ReferenceManager.instance.forgetPasswordManager.gameObject.SetActive(false); })*/);
            }
            if (codeValidateReponse.isError)
            {
                string reasons = "";
    ...
    public void ChangePassword()
    {
        ...
        onSuccess: async (response) =>
        {
            ...
            if (codeValidateReponse.isSuccess)
            {
                ReferenceManager.instance.PopupManager.Show("Password is Changed!", "Your Password Is Changed Successfully"/*, new IOSNativeAlert.AlertButton("Thank You", () => { ReferenceManager.instance.ChangePassSection.gameObject.SetActive(false); })*/);
            }
            if (codeValidateReponse.isError)
            {
                string reasons = "";
    ...
    public void DeleteAccount()
    {
        ...
        APIHandler.instance.Post("Auth/DeleteAccount", json, onSuccess: (response) =>
        {
            ...
            {
                ReferenceManager.instance.PopupManager.Show("Account Deleted!", "Your Account Is Deleted Successfully"/*, new IOSNativeAlert.AlertButton("Thank You", () => { Logout(); })*/);
            }
